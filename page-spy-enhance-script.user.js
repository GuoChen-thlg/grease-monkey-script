...
// @version             1.0.0
// @description         PageSpy 回放页日志筛选增强：类型/级别/关键词过滤，支持复制导出
// @author              THLG
...
(function () {
    ...
    GM_addStyle(
        ...
        '.ps-f-lv button.active.lv-error{background:#f38ba8;color:#1e1e2e}' +
        '.ps-f-lv button.active.lv-warn{background:#fab387;color:#1e1e2e}' +
        '.ps-f-lv button.active.lv-info{background:#89b4fa;color:#1e1e2e}' +
        '.ps-f-lv button.active.lv-log{background:#6c7086;color:#1e1e2e}' +
    ...
    function isReplayRoute() {
        return window.location.hash.indexOf('#/replay') === 0;
    }
    ...
    function fallbackCopy(text) {
        ...
        ta.select();
        try { document.execCommand('copy'); showToast('已复制 ' + filteredData.length + ' 条'); }
